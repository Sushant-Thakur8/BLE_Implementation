...
/* USER CODE BEGIN EV */
extern uint8_t Meter_Serial_Number_Received[9];
extern uint8_t instWBMBuffer[26];
/* USER CODE END EV */
  ...
  switch (p_event_pckt->evt)
  {
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        {
        	/* Comment added by @Sushant on //30-07-2024
        	 * Pairing_Req_Ret_Val is a variable for storing the return
        	 * value of aci_gap_peripheral_security_req() API.
        	 * This API will be used by the peripheral to notify, the central about the security requirements of the peripheral
        	 * The central(phone in our application) might encrypt the link, initiate the pairing procedure or reject the request
        	 */
          tBleStatus  Pairing_Req_RetVal;
          uint32_t conn_interval_us = 0;
  ...
  switch(ProcGapPeripheralId)
  {
    ...
    case PROC_GAP_PERIPH_ADVERTISE_NON_CONN_START_LP:
    {
      ...
      status = aci_gap_set_discoverable(paramD,
                                        ...
                                        CFG_BD_ADDRESS_TYPE,
                                        ADV_FILTER,
                                        0x09,(const uint8_t *)Meter_Serial_Number_Received, 0, 0, 0, 0);
      if (status != BLE_STATUS_SUCCESS)
      {
...
//       status = aci_gap_delete_ad_type(AD_TYPE_COMPLETE_LOCAL_NAME);
//       if (status != BLE_STATUS_SUCCESS)
//       {
//    	   LOG_INFO_APP("==>> delete complete local name - fail, result: 0x%02X\n", status);
//       }
      ...
      /* USER CODE BEGIN ADV_DATA_UPDATE_1 */
...
#if 0
       uint8_t AdvDataLen1=31;
       uint8_t AdvData1[AdvDataLen1];
       AdvData1[0]=0x02;
       AdvData1[1]=0x01;
       AdvData1[2]=0x06;
       AdvData1[3]= 27;
       ...
       memcpy(&AdvData1[5], &instWBMBuffer[0], 26);

       status = hci_le_set_advertising_data (AdvDataLen1, AdvData1);
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  if (role > 0)
  {
    ret = aci_gap_init(role,
                       CFG_PRIVACY,
					   sizeof(Meter_Serial_Number_Received)/sizeof(uint8_t),
                       &gap_service_handle,
    ...
    ret = aci_gatt_update_char_value(gap_service_handle,
                                     ...
                                     0,
                                     sizeof(Meter_Serial_Number_Received)/sizeof(uint8_t),
                                     (uint8_t *) &Meter_Serial_Number_Received[1]);
    if (ret != BLE_STATUS_SUCCESS)
    {
